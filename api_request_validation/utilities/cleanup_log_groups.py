...
class GlobalArgs:
    ...
    MODULE_NAME = "nuke_log_group"
    LOG_LEVEL = os.getenv("LOG_LEVEL", "INFO").upper()
    CW_LOG_GROUP_NAME = os.getenv(
        "CW_LOG_GROUP_NAME", "/aws/lambda/greeter_fn_api-request-validation")
...
def get_log_group_config(log_group_name):
    ...
    if len(log_groups) == 0:
        ...
        if log_groups[0]["logGroupName"] != log_group_name:
            ...
            raise Exception("More than one log group found, be more specific: {}\n"
                            " - Nuking a log group is pretty destructive, we will just do one at a time for now"
    ...
    if log_group_config["logGroupName"] != log_group_name:
        ...
        raise Exception("Log group found a single match but it was not quite right.\n"
...
def get_log_group_subscription_filters(log_group_name, next_token=None):
    ...
    subscription_filters_response = client.describe_subscription_filters(
        **opts)
...
def main(log_group_name, dry_run=False):
    ...
    # Fully pump our generator, we are deleting this resource so make sure we have everything first.
    ...
    if dry_run:
        print("Would create new log group: " +
              log_group_name + " (but --dry-run is set)")
    else:
    ...
    if dry_run:
        ...
        print("Setting expiration to: {} days".format(
            log_group_config["retentionInDays"]))
    ...
    for metric_filter in log_group_metric_filters:
        if dry_run:
            print(
                "Would put metric filter {} (but --dry-run is set)".format(metric_filter["filterName"]))
        else:
            print("Putting metric filter {}".format(
                metric_filter["filterName"]))
    ...
    for subscription_filter in log_group_subscription_filters:
        if dry_run:
            ...
            print("Putting subscription filter for {}".format(
...
def get_arg_parser():
    ...
    parser.add_argument("--dry-run",
                        dest="dry_run",
                        action="store_true",
                        help="Just print what we are going to do, do not actually do it."
                        )
